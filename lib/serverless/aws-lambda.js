...
class AwsLambda {
  ...
  wrapStreamAndCaptureError(transaction, txnEnder, stream, resultProcessor) {
    ...
    // 'prefinish' fires when stream.end() is called, but doing so leaves http statusCode undefined in agent attrs
    // 'close', 'finish', 'drain' are also events, but are emitted after the invocation is over
    const endEvents = ['close', 'finish', 'drain','prefinish']
    endEvents.forEach(ev => {
      stream.on(ev, () => {
        console.log('event arguments', arguments)
        console.log(`${ev} called; about to end transaction`)
        txnEnder()
      })
    })
    // instrumenting stream.end this way prevents us from setting http statusCode on agent attrs:
    // stream.end = this.wrapCallbackAndCaptureError(transaction, txnEnder, stream.end)
    return stream
  }

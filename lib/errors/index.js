...
class Exception {
  ...
  getErrorDetails(transaction, config) {
    const errorDetails = errorHelper.extractErrorInformation(transaction, this.error, config)
...
function createError(transaction, exception, config) {
  ...
  const { name, message, type } = errorHelper.extractErrorInformation(transaction, error, config)
  ...
  params.intrinsics[ERROR_EXPECTED_PATH] =
    exception._expected || errorHelper.isExpected(type, message, transaction, config)
